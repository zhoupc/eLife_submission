...
[d1,d2] = size(Cn);
imagesc(Cn,[min(Cn(:)),max(Cn(:))]); colormap(gca, 'gray'); 
...
if ~(nargin < 6 || isempty(Coor))
    ...
    for i = 1:size(Aor,2)
        ...
        if size(cont,2) > 1
            plot(cont(1,2:end),cont(2,2:end),'Color',cmap(i,:), 'linewidth', ln_wd); hold on;
        end
    end
else
...
if display_numbers
    ...
    text(round(cm(1:max_number,2)),round(cm(1:max_number,1)),lbl(1:max_number),'color', 'm','fontsize',12,'fontname',fontname,'fontweight','bold');
end
axis off;
if ~(nargin < 6 || isempty(Coor))
    jsf = [];
