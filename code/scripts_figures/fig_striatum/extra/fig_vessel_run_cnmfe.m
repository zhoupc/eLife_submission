...
nam = '~/Dropbox/github/drafts/CNMF-E/data/blood_vessel_6hz.tif'; 
cnmfe_choose_data;
...
thresh = inf;     % threshold for detecting frames with large cellular activity. (mean of neighbors' activity  + thresh*sn)
...
while miter <= maxIter
    ...
    for m=1:5    
        ...
        neuron.trimSpatial(0.05, 3); % for each neuron, apply imopen first and then remove pixels that are not connected with the center
...
%% order neurons 
temp = max(neuron.C,[], 2)'.* max(neuron.A,[], 1); 
...
Cnn = correlation_image(Ysignal, 8, d1, d2); 
neuron.Coor = plot_contours(neuron.A, Cnn, 0.6, 0, [], [], 2);
colormap winter;
axis equal; axis off;
...
plot_contours(neuron.A, Cn, 0.6, 0, [], [], 2);
colormap winter;
title('contours of estimated neurons');
