...
prepare_env;
output_folder = [fig_folder, filesep, 'Fig_Striatum_subfigs'];

results_folder = sprintf('%sfig_striatum%s', results_folder, filesep); 
if ~exist(results_folder, 'dir')
    mkdir(results_folder); 
end  
results_file = [results_folder, 'fig_striatum_results.mat'];
...
if export_fig
    ...
    saveas(gcf, sprintf('%s/variance_explained.png', output_folder));
end
...
ind_ica_miss(match_ica.ind_temporal) = [];
% ind_ica_miss = find(isnan(match_cnmfe.ind_max)); 
...
k = 0;
subplot(221); hold on;
imagesc(Cn_filter); axis equal off tight; colormap gray;
for m=1:K_ica
    temp = Coor_ica{m};
    temp = medfilt1(temp')';
    plot(temp(1,3:end), temp(2, 3:end), 'r');
end
for m=1:K_cnmfe
    temp = Coor_cnmfe{m};
    temp = medfilt1(temp')';
    plot(temp(1,3:end), temp(2, 3:end), 'g');
end
...
for m=(K_match+1):K_cnmfe
    ...
    temp = Coor_cnmfe{m};
    temp = medfilt1(temp')';
    h = fill(temp(1,3:end), temp(2,3:end), 'b', 'facealpha',0.3, 'edgecolor', 'none');
    axis equal off;
    ylim(ctr(m, 1)+[-20, 20]);
    xlim(ctr(m,2)+[-20, 20]);
    ...
    hold on; 
    plot(neuron.C(m, :), 'r'); 
    x = input('use this neuron as an example (y/n(default)): ', 's');
    if strcmpi(x, 'y')
        k = k+1;
        ind_miss(k) = m;
        if k==14
