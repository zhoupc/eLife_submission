...
dmin = 6;       % minimum distances between two neurons. it is used together with merge_thr
dmin_only = 2;  % merge neurons if their distances are smaller than dmin_only.
merge_thr_spatial = [0.7, -inf, 0.2];  % merge components with highly correlated spatial shapes (corr=0.8) and small temporal correlations (corr=0.1)

% -------------------------  INITIALIZATION   -------------------------  %
K = 500;             % maximum number of neurons per patch. when K=[], take as many as possible.
...
if with_manual_intervention
    show_merge = true;
    neuron.orderROIs('snrs');   % order neurons in different ways {'snr', 'decay_time', 'mean', 'circularity'}
